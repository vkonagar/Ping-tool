...
void send_packet_loop(int sock, struct sockaddr_in remote_host,int length,int seq_no)
{
...
int main()
{
	...
	for(;;)
	{
		...
		send_packet_loop(sock,remote_host,length,seq_no);
	...
	// If the packet is not received within 2 seconds, the select call returns, so that we can send one more packet
			...
			printf("Reply from %s , ttl = %d, checksum = %d, seq = %d RTT : %ld.%ld\n", \
					 ip, ip_h->ttl, hdr->checksum,\
